...
const feats = r => [r.temp / 100, clamp((r.prem_avg + 8) / 16, 0, 1), clamp(r.damian / 30, 0, 1), r.zb_rate / 100, clamp(r.max_lbc / 8, 0, 1)];
const F = feats(tgt);
...
console.log(`今日 ${tgt.date}: 温度 ${tgt.temp}(${tgt.state6}) | 溢价 ${tgt.prem_avg}% 红盘 ${tgt.prem_red}% 大面 ${tgt.damian} | 炸板率 ${tgt.zb_rate}% 高度 ${tgt.max_lbc}板`);
console.log(`\n最相似的 ${K} 天及其"次日"表现：`);
for (const x of near) { const n = nextOf(x.r.date); console.log(`  ${x.r.date}  距离${x.d.toFixed(2)}  温度${x.r.temp} 溢价${x.r.prem_avg}% 大面${x.r.damian}  →  次日 温度${n ? n.temp : '—'} 溢价${n ? n.prem_avg : '—'}% 红盘${n ? n.prem_red : '—'}% 大面${n ? n.damian : '—'}`); }
const nxt = near.map(x => nextOf(x.r.date)).filter(Boolean);
const dm = k => mean(nxt.map(n => n[k]).filter(v => v != null));
console.log(`\n相似日「次日」均值：溢价 ${dm('prem_avg')}% | 红盘 ${dm('prem_red')}% | 大面 ${dm('damian')} | 温度 ${dm('temp')}`);
